...
def restaurant_order_notification(request, last_request_time):
	notification = Order.objects.filter(
			...
			create_at__gt=last_request_time
			).count()
...
def driver_pick_order(request):
	if request.method == 'POST':
		...
		#check if driver can only pick one order at same time
		if Order.objects.filter(driver=driver).exlude(status = Order.ONTHEWAY):
			return JsonResponse({"status": "failed", "error": "You can only pick one order at same time"})
		try:
			...
			return JsonResponse({"status": "failed", "error": "This order has been pick up by another driver"})
